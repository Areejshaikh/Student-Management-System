...
let myId: number = Math.floor(10000 + Math.random() * 9000);
...
while (true) {
  ...
  console.log(
    `Selected Course: ${coursesData.courseInput} | Student ID: ${myId}`
  );
  ...
  if (tuitionFees === studentPay) {
    console.log(
      `Congratualation, SuccessFully enrolle in ${studentAnswer.courseInput}}`
    );
  }
